...
if __name__ == "__main__":
    ...
    # compute energies corresponding to lower and upper wavelengths
    # 1. convert input wavelength nm to m
    # 2. convert output energy from J to eV
    photonenergies = [(J2eV(photonenergycalc.photon_energy(lambdas[0] * 1e-9)),
                       J2eV(photonenergycalc.photon_energy(lambdas[1] * 1e-9)))
                      for name, lambdas in colors]

    # iterate through colors and photonenergies together (in reverse
    # order so that violet is at top) to pull out the relevant data
    # and pretty-print (note that lower energy belongs to higher
    # wavelength!)
